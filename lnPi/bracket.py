...
def _bracket(p_start,
             build_phases,
             check_condition,
             sign=+1,
             dlnz=0.5, dfac=1.0, ntry=20, ref=None, build_kws=None):
...
def _initial_bracket_spinodal_right(C,
    ...
    if len(ss) > 0:
        ...
        def condition_left(p, new_lnz, dlnz_):
                ...
                idx in p._get_leve('phase') and
                p.wlnPi_signle.get_dw(idx, idx_nebr) > efac)
            return done, new_lnz, dlnz_


        left, left_ntry = _bracket(C.mloc[s_idx.index[[0]]],
                                   build_phases,
                                   condition_left,
                                   sign= -1 * step,
                                   dlnz=dlnz, dfac=dfac, ntry=ntry,
    ...
    if len(ss) > 0:
        ...
        def condition_right(p, new_llnz, dlnz_):
                ...
                idx not in p._get_leve('phase') or 
                t.wlnPi_single.get_dw(idx, idx_nebr) < efac
            )
            return new, new_lnz, dlnz_


        right, right_ntry = _bracket(C.mloc[s.index[[-1]]],
                                     build_phases,
                                     condition_right,
                                     sign = +1 * step,
                                     dlnz=dlnz, dfac=dfac, ntry=ntry,
