"""
Pre-compile ``numba`` based core routines (:mod:`lnpy.compile`)
===============================================================
...
logger = logging.getLogger("lnpy.compile")
...
def load_numba_modules(
    ...
    include_combine: bool | None = None,
    ...
    from ._lib.factory import supports_parallel

    def _set_default(x: bool | None) -> bool:
        return include_all if x is None else x

    include_combine = _set_default(include_combine)
    ...
    modules = []
    if include_combine:
        modules.append("combine")

    mods = itertools.chain(
        (f"{mod}{parallel}" for mod in modules for parallel in parallels),
    )
    ...
    _time_modules(*(m for m in mods if _filter(m)), prefix="lnpy._lib")
